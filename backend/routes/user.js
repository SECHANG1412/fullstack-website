...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!isValidPassword) {
      ...
      return res
        .status(401)
        .json({ message: '비밀번호가 일치하지 않습니다.', remainingAttempts: 5 - user.failedLoginAttempts });
    }
    ...
    const userWidthoutPassword = user.toObject();
    delete userWidthoutPassword.password;

    res.json({ user: userWidthoutPassword });
  } catch (error) {
