          ...
          <tbody>
            {dummyPosts.map((post, index) => (
              <tr key={post._id} className="border-b">
                ...
                <td className="px-4 py-3 text-center">
                  {post.fileUrl.length > 0 ? (
                    post.fileUrl.map((url, index) => (
                      <button
                        key={index}
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-50 hover:bg-gray-700 text-sm rounded-md transition-300 duration-300 border border-gray-200 hover:border-gray-300 mr-2"
                      >
                        파일 {index + 1}
                      </button>
                    ))
                ...
                <td className="px-4 pt-3">{new Date(post.createdAt).toLocaleString()}</td>
                <td className="px-4 pt-3">{new Date(post.updatedAt).toLocaleString()}</td>
